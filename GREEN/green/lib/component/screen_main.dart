...
import 'package:green/models/model_logdata.dart';
...
class ScanscreenState extends State<Scanscreen> {
  ...
  String center_uri = '';
  String ip_number = '';
  String port_number_list = '';
  int port_number = 0;
  String real_phoneNumber = '';
  String app_name = '';
  bool app_name_2 = false;
...
// getCurrentLocation();
  ...
  Future<bool> checkCurrentAppVersion() async {
    ...
    var uri = Uri.parse('http://gep.thermocert.net:17001/version_checker');
    var uriResponse = await client.post(uri, body: {"app_name": 'GEP_J'});
  ...
  FIRST_INFO() async {
    ...
    var client = http.Client();
    var uri = Uri.parse('http://gep.thermocert.net:17002/FIRST_INFO');
    var uriResponse = await client.post(uri, body: {"ZONE_NM": phoneNumber});
    print(uriResponse.body);
    print("제발 되세요 ///////////////////////////////////////////////");

    List<dynamic> list = jsonDecode(uriResponse.body.substring(43));

    print(list[0]['center_name']);

    setState(() {
      app_name = uriResponse.body.substring(35, 36);
      center_name = list[0]['center_name'];
      center_uri = list[0]['center_uri'];
      ip_number = list[0]['ip_number'];
      port_number_list = "http://" +
          list[0]['ip_number'] +
          ":" +
          list[0]['port_number_list'] +
          "/" +
          list[0]['center_uri']; // http://175.126.232.236:8985/GP
      port_number = list[0]['port_number'];
  ...
  getDeviceList() async {
    ...
    try {
      ...
      var uri = Uri.parse(port_number_list);

      var uriResponse = await client.post(uri,
          headers: {"Content-Type": "application/x-www-form-urlencoded"},
          body: {"ZONE_NM": real_phoneNumber});
      ...
      if (uriResponse.statusCode == 200) {
        ...
        if (beforePhoneNumber == '') {
          ...
          if(app_name == 'J'){
            startGPSTimer(real_phoneNumber);
            startTimer();
            init();
            beforePhoneNumber = real_phoneNumber;
          }else {
            startTimer();
            init();
            beforePhoneNumber = real_phoneNumber;
  ...
  Future<String> sendtoServer(List<LogData> list, String devicename,
    ...
    Socket socket = await Socket.connect(ip_number, port_number);
    if (socket != null) {
      bool isOver = false;
  ...
  Future<String> sendGPStoServer(String phoneNumber) async {
    ...
    if (socket != null) {
      ...
      print(currentLocation.latitude.toString() +
          '|' +
          currentLocation.longitude.toString());
      print(body + ip_number + port_number.toString());
      socket.close();
      return 'success';
    } else {
      print('Fail Send to Server');
    ...
    monitoringStreamSubscription = characteristicUpdates.listen(
        ...
        if (notifyResult[10] == 0x03) {
          ...
          if (index != -1) {
            ...
            int tempstamp = threeBytesToint(minmaxStamp.sublist(3, 6)) - 540;
            if (tempstamp <= 0) tempstamp += 540;
                        ...
                        title: Row(
                          ...
                          children: [
                            Expanded(
                              flex: 5,
                              child: Image(
                                image: AssetImage('images/gep_logo.png'),
                                fit: BoxFit.contain,
...
showMyDialog_end(
    BuildContext context, String destName, String de_name, String devicename,  String ip_number, int port_number) {
                      ...
                      children: [
                        InkWell(
                          onTap: () async {
                            ...
                            if (result == 1) {
                              Socket socket =
                              await Socket.connect(ip_number, port_number);
                              if (socket != null) {
                                String body = '';
                                body += devicename +
                                    '||' +
